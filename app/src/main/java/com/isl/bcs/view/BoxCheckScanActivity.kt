...
class BoxCheckScanActivity : BaseActivity() {

    private lateinit var mBinding: ActivityBoxScanBinding

    /**
     * 扫描处理
     */
    private var mHandler: BoxCheckScanActivity.CaptureActivityHandler? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding = ActivityBoxScanBinding.inflate(layoutInflater)
        with(mBinding) {
            setContentView(this.root)
            immersive(toolbar.toolbar, false)
            setSupportActionBar(toolbar.toolbar)
            supportActionBar?.title = ""
            toolbar.tvTitle.text = getString(R.string.box_label_scan)
            imageView5.setImageResource(R.mipmap.home_icon_3)
            //界面控件初始化
            initDecode()
            //权限初始化
            initPermission()
            //扫描动画初始化
            //初始化 CameraManager
            CameraManager.init(this@BoxCheckScanActivity)
            hasSurface = false
            btnStart.setOnClickListener {
                initScanerAnimation()
                mHandler?.sendEmptyMessage(restartPreview)
            }
    ...
    private fun initCamera(surfaceHolder: SurfaceHolder) {
        ...
        if (mHandler == null) {
            mHandler = CaptureActivityHandler()
        }
    }
    ...
    fun handleDecode(result: Result) {
        ...
        val res = result.text
        Log.e("二维码/条形码 扫描结果", res)
        val boxInRes = LitePal.where("BOX_LABEL1 = ?", res).find<InstItemCheck>()
        if (boxInRes.isNotEmpty()) {
            openActivity<BoxCheckPreviewActivity>(
                "boxData" to boxInRes[0],
            )
        } else {
    ...
    internal inner class CaptureActivityHandler : Handler() {
        ...
        override fun handleMessage(message: Message) {
            if (message.what == autoFocus) {
                ...
                mBinding.captureScanLine.clearAnimation()
...
//            restartPreviewAndDecode()
    ...
    private fun decode(data: ByteArray, width: Int, height: Int) {
        ...
        if (rawResult != null) {
            val message = Message.obtain(mHandler, decodeSucceeded, rawResult)
            val bundle = Bundle()
            bundle.putParcelable("barcode_bitmap", source.renderCroppedGreyscaleBitmap())
            message.data = bundle
            message.sendToTarget()
        } else {
            val message = Message.obtain(mHandler, decodeFailed)
            message.sendToTarget()
        }
    }
