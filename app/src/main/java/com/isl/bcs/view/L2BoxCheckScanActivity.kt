...
class L2BoxCheckScanActivity : BaseActivity() {
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(p0: Context?, p1: Intent?) {
            ...
            val instItemCheck = LitePal.where("INST_IN_KEY = ?", code).find<InstItemCheck>()
            if (instItemCheck.isNotEmpty()) {
                openActivity<BoxCheckPreviewActivity>(
                    "boxData" to instItemCheck[0]
                )
            }else{
                RxDialogSure(this@L2BoxCheckScanActivity).apply {
