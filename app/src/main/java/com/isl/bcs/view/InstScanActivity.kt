...
import com.isl.bcs.databinding.ActivityInstScanBinding
...
class InstScanActivity : BaseActivity() {

    private lateinit var mBinding: ActivityInstScanBinding

    /**
     * 扫描处理
     */
    private var handler: InstScanActivity.CaptureActivityHandler? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding = ActivityInstScanBinding.inflate(layoutInflater)
        with(mBinding) {
            ...
            //扫描动画初始化
            //初始化 CameraManager
            CameraManager.init(this@InstScanActivity)
            hasSurface = false
...
//        toast("Scan result : $res")
        val split = res.split(",")
        if (split.isEmpty() || split.size < 12) {
            RxDialogSure(this@InstScanActivity).apply {
