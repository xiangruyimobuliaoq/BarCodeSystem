...
public class SunMiStaffScanActivity extends BaseActivity implements SurfaceHolder.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sunmi_staff_scan);
        setImmersive(false);
        setSupportActionBar(findViewById(R.id.toolbar));
        getSupportActionBar().setTitle("");
        ((TextView) findViewById(R.id.tv_title)).setText(getString(R.string.staff_scan));
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        try {
            verifyStoragePermissions(SunMiStaffScanActivity.this);
        } catch (Exception e) {
            mCamera = null;
        }
    }
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if (nsyms != 0) {
                ...
                for (Symbol sym : syms)//如果允许识读多个条码，则解码结果可能不止一个
                {
                    ...
                    List<Staff> staff = LitePal.where("id_no = ? ", sym.getResult()).find(Staff.class);
                    if (staff.size() > 0) {
                        Constants.Companion.setCurrentStaff(staff.get(0));
                        switch (getIntent().getStringExtra(Constants.INTENT_TYPE)) {
                            case Constants.TYPE_INOUT:
                                startActivity(new Intent(SunMiStaffScanActivity.this, InOutActivity.class));
                                break;
                            case Constants.TYPE_TRANSFER:
                                startActivity(new Intent(SunMiStaffScanActivity.this, TransferActivity.class));
                                break;
                            case Constants.TYPE_CHECK:
                                startActivity(new Intent(SunMiStaffScanActivity.this, CheckActivity.class));
                                break;
                        }
                        finish();
                    } else {
                        RxDialogSure rxDialogSure = new RxDialogSure(SunMiStaffScanActivity.this);
                        rxDialogSure.setContent(getString(R.string.staff_code_error));
