...
class SunMiBoxTransferScanActivity : BaseActivity(), SurfaceHolder.Callback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        (findViewById<View>(R.id.imageView5) as ImageView).setImageResource(R.mipmap.home_icon_2)
        init()
    }
    ...
    override fun surfaceCreated(holder: SurfaceHolder) {
        try {
            verifyStoragePermissions(this@SunMiBoxTransferScanActivity)
        } catch (e: Exception) {
            mCamera = null
        }
    }
    ...
    var previewCallback = Camera.PreviewCallback { data, camera ->
        ...
        if (nsyms != 0) {
            ...
            {
                val boxInRes = LitePal.where("BOX_LABEL1 = ?", sym.result).find<InstItemOut>()
